...
Motion should be driven by three layers:
...
- **Semantic calibration**: asynchronous LLM analyzer results. It corrects emotion and intensity over time without direct snapping.
...
- Trigger every `semanticIntervalMs`, default `1000`.
...
- Configurable `stability`, default `0.84`.

Body motion should be part of the same frame generation:

- `ParamBodyAngleX/Y/Z`
- `ParamAngleX/Y/Z`
- eye open and eye ball parameters
- mouth form and mouth open
...
3. Run `/api/analyze` as background semantic calibration, not as a blocking final step.
